...
The installer intentionally reads immutable release tags from GitHub and never falls back to `latest`. Anonymous tag discovery works after the repository is public. Until then, authenticated maintainers can run a local script with an explicit stable tag:
